...
void cusz::impl::GatherAsCSR(FP* d_A, size_t lenA, size_t ldA, size_t m, size_t n, int* nnz, std::string* fo)
{
    uint8_t* outbin;
...
template <typename FP>
void cusz::impl::ScatterFromCSR(FP* d_A, size_t lenA, size_t ldA, size_t m, size_t n, int* nnz, std::string* fi)
{
    // clang-format off
    auto lrp         = sizeof(int) * (ldA + 1);
    auto lci         = sizeof(int) * *nnz;
    auto lv          = sizeof(FP)  * *nnz;
    ...
    auto csr_val     = reinterpret_cast<FP* >(outlier_bin + lrp + lci);  // TODO template
    // clang-format on
